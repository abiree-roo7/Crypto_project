...
SESSIONS_JSON = BASE_DIR / "sessions_tls.json"
...
def load_pipeline_module():
    spec = importlib.util.spec_from_file_location("tls_ids_pipeline", BASE_DIR / "code.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def run_pipeline_for_capture(capture_path):
...
<body>
  ...
  <main class="wrap">
    ...
    <form class="actions" method="post" action="{{ url_for('run_capture') }}" enctype="multipart/form-data">
      <input name="capture" type="file" accept=".pcap,.pcapng,.cap" aria-label="PCAP capture">
...
def run_capture():
    ...
    if not uploaded or not uploaded.filename:
        flash("Choose a PCAP, PCAPNG, or CAP file before running the pipeline.")
        return redirect(url_for("dashboard"))

    original_name = secure_filename(uploaded.filename)
    extension = Path(original_name).suffix.lower()
    if extension not in ALLOWED_CAPTURE_EXTENSIONS:
        flash("Unsupported capture file. Use .pcap, .pcapng, or .cap.")
        return redirect(url_for("dashboard"))

    UPLOAD_DIR.mkdir(exist_ok=True)
    capture_path = UPLOAD_DIR / original_name
